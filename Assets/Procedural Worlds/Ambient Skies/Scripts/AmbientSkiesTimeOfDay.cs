...
namespace AmbientSkies
{
    ...
    public class AmbientSkiesTimeOfDay : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                m_timeOfDayProfile.m_renderPipeline = AmbientSkiesConsts.RenderPipelineSettings.Universal;
            }
        ...
        public void UpdateHighDefinition(float gameTime)
        {
            ...
            else
            {
                ...
                {
                    if (volumeProfile.name != "2019.3 HDRP Sky Profile")
                    {
                ...
                if (volumeProfile != null)
                {
                    ...
                    if (volumeProfile.TryGet(out physicallyBasedSky))
                    {
                        ...
                        if (m_timeOfDayProfile.m_pBSGroundColor != null)
                        {
                            physicallyBasedSky.groundTint.value = m_timeOfDayProfile.m_pBSGroundColor.Evaluate(lightRotationDot);
                        }

                        if (m_timeOfDayProfile.m_pBSGroundColor != null)
                        {
                            physicallyBasedSky.aerosolDensity.value = m_timeOfDayProfile.m_pBSAreosolDensity.Evaluate(lightRotationDot);
                        }

                        physicallyBasedSky.spaceEmissionTexture.value = m_timeOfDayProfile.m_pBSSpaceEmissionTexture;
                        physicallyBasedSky.spaceEmissionMultiplier.value = m_timeOfDayProfile.m_pBSSpaceEmissionMultiplier;
                    }
                    ...
                    if (volumeProfile.TryGet(out fog))
                    {
                        ...
                        if (m_sunActive)
                        {
                            if (m_timeOfDayProfile.m_dayFogColor != null)
                            {
                                fog.albedo.value = m_timeOfDayProfile.m_dayFogColor.Evaluate(lightRotationDot);
                            }

                            if (m_timeOfDayProfile.m_dayFogDistance != null)
                            {
                                fog.meanFreePath.value = m_timeOfDayProfile.m_dayFogDistance.Evaluate(lightRotationDot);
                            }

                            if (m_timeOfDayProfile.m_lightAnisotropy != null)
                            {
                                fog.anisotropy.value = m_timeOfDayProfile.m_lightAnisotropy.Evaluate(lightRotationDot);
                            }

                            if (m_timeOfDayProfile.m_lightProbeDimmer != null)
                            {
                                fog.globalLightProbeDimmer.value = m_timeOfDayProfile.m_lightProbeDimmer.Evaluate(lightRotationDot);
                            }
                        }
                        ...
                        else
                        {
                            if (m_timeOfDayProfile.m_nightFogColor != null)
                            {
                                fog.albedo.value = m_timeOfDayProfile.m_nightFogColor.Evaluate(lightRotationDot);
                            }

                            if (m_timeOfDayProfile.m_nightFogDistance != null)
                            {
                                fog.meanFreePath.value = m_timeOfDayProfile.m_nightFogDistance.Evaluate(lightRotationDot);
                            }
                            ...
                            {
                                fog.depthExtent.value = m_timeOfDayProfile.m_lightDepthExtent.Evaluate(lightRotationDot);
                            }
                        }
                    }
                }
...
#else
                    volume.sharedProfile = AssetDatabase.LoadAssetAtPath<VolumeProfile>(GetAssetPath("2019.3 HDRP Sky Profile"));
#endif
#endif
                    //Gets the profile
