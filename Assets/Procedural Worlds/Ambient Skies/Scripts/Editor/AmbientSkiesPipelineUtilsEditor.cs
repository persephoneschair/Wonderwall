...
namespace AmbientSkies
{
    ...
    public class AmbientSkiesPipelineUtilsEditor : EditorWindow
    {
        ...
        private void RemoveObjectComponents()
        {
            switch (m_pipelineToInstall)
            {
                case AmbientSkiesConsts.RenderPipelineSettings.BuiltIn:
                    ...
                    SkyboxUtils.DestroyParent("Ambient Skies Global", true);
                    ...
                    UnityEngine.Rendering.Universal.UniversalAdditionalCameraData lWRPAdditionalCameraData = FindObjectOfType<UnityEngine.Rendering.Universal.UniversalAdditionalCameraData>();
                if (lWRPAdditionalCameraData != null)
                {
                    DestroyImmediate(lWRPAdditionalCameraData);
                }

                UnityEngine.Rendering.Universal.UniversalAdditionalLightData lWRPAdditionalLightData = FindObjectOfType<UnityEngine.Rendering.Universal.UniversalAdditionalLightData>();
...
#if LWPipeline && UNITY_2018_3_OR_NEWER
                    UnityEngine.Rendering.Universal.UniversalAdditionalCameraData lWRPAdditionalCameraDataHD =FindObjectOfType<UnityEngine.Rendering.Universal.UniversalAdditionalCameraData>();
                if (lWRPAdditionalCameraDataHD != null)
                {
                    DestroyImmediate(lWRPAdditionalCameraDataHD);
                }
                UnityEngine.Rendering.Universal.UniversalAdditionalLightData lWRPAdditionalLightDataHD = FindObjectOfType<UnityEngine.Rendering.Universal.UniversalAdditionalLightData>();
        ...
        private void LoadPackageList()
        {
            ...
            else if (m_pipelineToInstall == AmbientSkiesConsts.RenderPipelineSettings.Universal)
            {
                ...
                switch (m_pipelineToInstall)
                {
                    case AmbientSkiesConsts.RenderPipelineSettings.BuiltIn:
                        ...
                        if (m_finalizeEnvironment)
                        {
                            ...
                            {
                                PostProcessingUtils.SetFromProfileIndex(m_profiles, m_selectedPostProcessingProfileIndex, false, m_updateAO, m_updateAutoExposure, m_updateBloom, m_updateChromatic, m_updateColorGrading, m_updateDOF, m_updateGrain, m_updateLensDistortion, m_updateMotionBlur, m_updateSSR, m_updateVignette, m_updatePanini, m_updateTargetPlaform);
                            }
                            else
                            {
                                if (m_showDebug)
