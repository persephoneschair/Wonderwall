...
namespace AmbientSkies
{
    ...
    public static class LightingUtils
    {
        ...
        public static void SetFromProfileIndex(AmbientSkyProfiles skyProfiles, int profileIndex, bool useDefaults, bool updateRealtime, bool updateBaked)
        {
            if (skyProfiles == null)
            {
            ...
            AmbientLightingProfile profile = skyProfiles.m_lightingProfiles[profileIndex];
            if (profile == null)
            {
            ...
            SetLightmapSettings(skyProfiles, profile, false, updateRealtime, updateBaked);
        ...
        public static void SetLightmapSettings(AmbientSkyProfiles skyProfiles, AmbientLightingProfile profile, bool useDefaults, bool updateRealtime, bool updateBaked)
        {
            LightingSettings settings = AssetDatabase.LoadAssetAtPath<LightingSettings>(SkyboxUtils.GetAssetPath("Ambient Skies Lightmap Settings.asset"));
            if (!Lightmapping.TryGetLightingSettings(out settings))
            {
            ...
            Lightmapping.lightingSettings.albedoBoost = profile.lightBoostIntensity;
            Lightmapping.lightingSettings.indirectScale = profile.lightIndirectIntensity;
        ...
        public static void SetRealtimeGISettings(AmbientLightingProfile profile)
        {
            ...
            Lightmapping.lightingSettings.indirectResolution = profile.indirectRelolution;
            if (profile.useDirectionalMode)
            {
                LightmapSettings.lightmapsMode = LightmapsMode.CombinedDirectional;
            }
        ...
        public static void SetBakedGISettings(AmbientLightingProfile profile)
        {
            ...
            Lightmapping.lightingSettings.lightmapResolution = profile.lightmapResolution;
            Lightmapping.lightingSettings.lightmapPadding = profile.lightmapPadding;
            Lightmapping.lightingSettings.filteringMode = profile.filterMode;

            switch (profile.lightmappingMode)
            {
                case AmbientSkiesConsts.LightmapperMode.Enlighten:
                    Lightmapping.lightingSettings.lightmapper = LightingSettings.Lightmapper.Enlighten;
                    break;
                case AmbientSkiesConsts.LightmapperMode.ProgressiveCPU:
            ...
            if (profile.useHighResolutionLightmapSize)
            {
                Lightmapping.lightingSettings.lightmapMaxSize = 4096;
            }
            else if (!profile.useHighResolutionLightmapSize)
            {
            ...
            Lightmapping.lightingSettings.compressLightmaps = profile.compressLightmaps;
            Lightmapping.lightingSettings.ao = profile.ambientOcclusion;
            Lightmapping.lightingSettings.aoMaxDistance = profile.maxDistance;
            Lightmapping.lightingSettings.aoExponentIndirect = profile.indirectContribution;
            Lightmapping.lightingSettings.aoExponentDirect = profile.directContribution;
        }
        /// <summary>
        ...
        public static void CreateLightingSettingsAsset()
        {
            ...
            LightingSettings lightingSettings = AssetDatabase.LoadAssetAtPath<LightingSettings>(SkyboxUtils.GetAssetPath("Ambient Skies Lightmap Settings.asset"));
            ...
            {
                Lightmapping.lightingSettings = new LightingSettings() { name = "Ambient Skies Lightmap Settings" };
                string path = "Assets/Ambient Skies Lightmap Settings.asset";
                AssetDatabase.CreateAsset(Lightmapping.lightingSettings, path);
        ...
        {
            GameObject theParent = SkyboxUtils.GetOrCreateParentObject("Ambient Skies Global", false);
